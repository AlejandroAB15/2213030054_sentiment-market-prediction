...
def run_preprocesado():
    ...
    with open(output_dir / "dataset_relevante.json", "w", encoding="utf-8") as f:
        json.dump(df_relevantes.to_dict("records"), f, ensure_ascii=False, indent=2)

    with open(output_dir / "dataset_no_relevante.json", "w", encoding="utf-8") as f:
        json.dump(df_no_relevantes.to_dict("records"), f, ensure_ascii=False, indent=2)

    log_and_print(logger, "[PREPROCESADO] Datasets exportados ruta final")
